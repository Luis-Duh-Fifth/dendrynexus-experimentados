...
(function($) {
  ...
  BrowserUserInterface.prototype.displayMyNewRow = function(items) {
    if (window && window.displayMyNewRow) {
        window.displayMyNewRow(items);
        return null;
    }
    this.$content.append($('<hr>'));
        var description = 'My Row - click to use.';
if (window.myRowDescription) {
    description = window.myRowDescription;
}
if (this.dendryEngine.state.qualities.myRowDescription) {
    description = this.dendryEngine.state.qualities.myRowDescription;
}
    this.$content.append($('<p>').addClass('my-row-description').text(description));
    var $rowEl = $('<ul>').addClass('my-row');
    for (var item of items) {
        var $itemEl = $('<li>').addClass('my-row-item');
        var $itemLink = $('<a>').addClass('card').attr({href: '#', 'card-id': item.id, title: item.title});
        var $title = $('<span>').addClass('card-caption').text(item.title);
        if (item.image) {
            var $img = $('<img>').addClass('card-img').attr({src: item.image});
            $itemLink.append($img);
        }
        if (item.subtitle) {
            var $subtitle = $('<span>').addClass('card-tooltip').text(item.subtitle);
            $itemLink.append($subtitle);
        }
        $itemEl.append($itemLink);
        $itemEl.append($title);
        $rowEl.append($itemEl);
    }
    this.$content.append($rowEl);
};
  ...
  BrowserUserInterface.prototype._registerEvents = function() {
    ...
    this.$content.on('click', 'ul.my-row li a', function(event) {
    event.preventDefault();
    event.stopPropagation();
    var choice = $(this).attr('card-id');
    that.dendryEngine.playPinnedCard(choice);
